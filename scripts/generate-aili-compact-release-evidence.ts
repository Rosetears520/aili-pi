...
function scanFlags(bodies: readonly string[]): Record<string, boolean> {
 ...
 * migration results: missing, stale, or NON_PASS inputs keep the index and
 * sanitizer explicitly NON_PASS.
 */
export async function generateAiliCompactReleaseEvidence(
  ...
  const [provenanceText, sbomText, noticesText] = await Promise.all([
  ...
  const flags = scanFlags([...bodies.values()]);
  const scannedArtifacts = [...bodies.keys()].map((id) => AILI_COMPACT_RELEASE_ARTIFACTS[id].path);
  ...
  const candidateReady = requiredIds.every((id) => candidateBound(id, values.get(id), binding));
