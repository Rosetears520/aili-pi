...
The script preserves an existing compatible Pi by default. After the Package install succeeds, it atomically merges `"compaction": { "enabled": false }` into user-global `~/.pi/agent/settings.json`, preserving unrelated settings. This gives AILI Compact exclusive context-management ownership. Malformed or non-object settings fail without replacing the original file. Project `.pi/settings.json` files are never scanned or rewritten. A direct `pi install` does not perform this bootstrap configuration; set the same Pi option yourself or rerun `./install.sh`.
...
Pi `/compact` is redirected to AILI controls whenever AILI is enabled. If AILI's independent GC cannot recover enough budget, the provider overflow is surfaced truthfully; Pi summary generation and compact-and-retry are not used. `/aili-compact off` does not silently re-enable Pi auto-compaction, so users who disable AILI must explicitly choose another compaction owner. Local serializer evidence covers installed OpenAI Completions/Responses, Anthropic, and Gemini adapters, but real-provider quality/cache performance, later third-party context-handler order, and Pi internal request order remain Unverified. The package-wide AGPL disposition is resolved for 0.1.13; these independent live-evidence gates remain non-pass until separately verified.
