...
AILI SHALL NOT implement clipboard-image reading, PowerShell image extraction, decoding, resizing, or attachment insertion. The keybinding SHALL dispatch Pi 0.82.1's existing `app.clipboard.pasteImage` action through the wrapped editor's forwarded `onPasteImage`. File drag-in SHALL remain unchanged.
