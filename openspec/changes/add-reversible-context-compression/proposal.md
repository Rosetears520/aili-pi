...
- 当前 Package 仍为 MIT。用户已批准 0.1.11 采用 ACP v1.12.6 reference-only 路线：记录精确 revision/license，但不复制或分发 ACP source、prompt、schema、fixture 或 asset。
- 0.1.11 的 version、commit、push、publish 和本机安装已获明确批准，但仍须通过 fresh release checks；任何未来 AGPL-derived source copy 或新增 dependency/lockfile mutation 仍需单独精确批准。
