...
Doctor health is not satisfied merely because `/aili-compact` is registered. It reports bounded component status for reducer/replay schema, reference catalog, projection/recap invariants, native-compaction hooks, cache telemetry availability, prompt slot count/hash and release disposition. A known projection/replay invariant failure is `ERROR`; missing optional/live evidence is `WARN`/`Unverified`; the resolved MIT/reference-only release route is PASS only while exact provenance and no-copy boundaries remain present.

## Reference-source and release boundary

The reference source is AGPL-3.0-or-later. This repository remains MIT and releases AILI Compact through the user-approved reference-only route: exact ACP revision/license attribution is recorded, but no ACP source, source-derived asset, prompt, schema or fixture is copied or distributed. Any future direct adaptation still requires a new exact approval and compatible provenance/license treatment. The approved corrective 0.1.12 version, Git push, npm publish and local install remain conditional on fresh release checks; this does not weaken fail-open, source-fidelity or test requirements.
