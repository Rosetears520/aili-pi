...
- Keep Pi JSONL as conversation truth and expose Web state through a versioned snapshot/event API. Inside the Web application, one AILI Runtime Gateway/BFF is the sole mutation owner; browser UI code and legacy API routes may not invoke mutable Pi, filesystem, Git, Worktree, Agent, MCP, Analytics, BTW, Stamp, model/plugin/skill, or media owners directly.
...
- Align the active Web baseline to exact `@agegr/pi-web@0.8.11` tag revision `28bab3c25f5f6770c9b0b745ebbfec1c27f7b948` (npm gitHead `024be0b1154ba8a2650237a2db8bfa89124e167e`) and official Pi `0.84.4`; retain 0.8.9 only as historical source evidence, Node.js `>=22.19.0`, MIT primary licensing, one owned Pi Extension entry, and the existing `aili-workflows` semantic-ownership boundary.
...
- `shared-pi-session-web-runtime`: Pi JSONL session truth plus one Web-side mutation owner, safe release, crash recovery, and explicit non-support for the retired stock-TUI/Web observer attachment contract.
...
- **Non-goals:** no Pi fork or replacement CLI, multi-user service, direct public-Internet support claim, cloud sync, full browser IDE/terminal, plugin marketplace, large Agent graph, remote Agent fleet, complete mobile parity, MCP configuration editor, hidden daemon, or copied Codex/`pi-gui`/OpenCode runtime/source. A native agent-driven Browser extension is deferred to a later optional change and requires fresh explicit authorization; it is not part of this BUILD scope.
