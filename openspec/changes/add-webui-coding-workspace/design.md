...
- FileTree formalization in `FileExplorer` (amended by user direction 2026-08-19: extend existing chrome, never duplicate it): the sidebar already owns the explorer surface — project path display and branch/worktree switching above the tree, and the explorer toolbar's refresh (with done indicator), upload, and changed-count controls. The tree adds no header chrome of its own; badges, expand/collapse, selected state, and changed-directory highlighting already exist and stay. The capability's "manual refresh and current-cwd display" requirements are satisfied by those existing sidebar controls. All access stays on `/api/files` + allowed-roots.
