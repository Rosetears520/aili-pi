...
This change makes the user’s backend choice durable and gives registered Parent-selected external CLI work the same observable subagent lifecycle as normal Pi subagents.
...
- Add an external-CLI Herdr driver for `claude-code`, `codex-cli`, `opencode`, `grok-cli`, and `agy-cli`; consumer Gemini CLI is intentionally excluded in favor of Agy. It uses the normal Pi-subagent tab/pane allocator, but each running external CLI Agent owns its active pane.
