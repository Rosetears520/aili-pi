...
- BUILD readiness：`COMPLETE`——accepted implementation and final evidence reconciliation are closed; SHIP and every remaining risky operation retain separate fresh-intent/exact-approval gates.
...
当前 readiness：`BUILD_COMPLETE`；SHIP 未开始，任何后续 lock/snapshot mutation、删除、external write、Git、publish、release 与真实安装仍保持各自独立门禁。
