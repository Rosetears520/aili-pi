...
- [x] 8.1 Expose formal ready-package planning and explicit restart reconciliation through parent/ROSE-only `hub` actions loaded by the Extension, preserving explicit operation/ownership/write-scope evidence and zero automatic reconciliation or phase movement.
- [x] 8.2 Reconcile generated ordinary role JSON guidance with the exact formal `CANONICAL RESULT:` assignment override, update the role manifest contract, and cover the real generated role plus Runtime prompt/parser path.
- [x] 8.3 Inspect same-Agent/same-formal-identity continuation turns during restart reconciliation; preserve queued/running follow-ups, block failed/aborted/interrupted follow-ups, and reject completed follow-ups without fresh immutable canonical evidence instead of reusing the initial result.
- [x] 8.4 Require non-empty normalized formal write scope for mutation-capable effective roles in the public validation/allocation path and propagate validated scope through planning, continuation identity, workspace protection, and focused in/out/owning-file checks.
- [x] 8.5 Restore the 0.1.16 prepublish role/routing, compatibility, provenance, bundle, package and loaded-runtime gates without reintroducing the historical exact-0.1.15 interim validator; correct repository-only versus installed-package documentation.
- [x] 8.6 Complete the final command gate.
  - Completed 2026-08-01 by ROSE: `prepublishOnly`, typecheck, strict OpenSpec, diff hygiene, focused repair/security tests, full suite, Linux disposable lifecycle, actual candidate packing and bounded first-party exposure scan pass. Git/publish/install operations remain pending.
