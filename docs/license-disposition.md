# License disposition for `@rosetears/aili-pi@0.2.9`
...
| `skills/`, `roles/`, `upstream/aili-workflows-runtime/` | `rose-aili@0.4.8` / AILI Workflows | MIT; exact/adapted boundaries recorded in `manifests/provenance.json` |
...
| `upstream/pi-codex-compact-*`, npm dependency | `@narumitw/pi-codex-compact@0.52.0`, commit `04aae270c51cf4de70479d84317eb15ac8e20e33` | MIT; exact active source/license snapshot plus locked dependency; superseded 0.50.0 evidence is not retained |
| npm dependency | root `@narumitw/pi-tui-kit@0.60.0`, commit `a96c77a6415076182c6817d2abd6739e59418401`; Codex nested `0.59.0` | MIT; root supports current File Context seams while Codex keeps its declared `^0.59.0`-compatible nested copy without override |
...
| Herdr integration (`src/runtime/persistent-agents/backends/herdr/`, `src/runtime/persistent-agents/herdr-child/`) | First-party code integrating the user-installed `herdr` runtime (Apache-2.0; verified 0.8.x / protocol 20) | No herdr source or binaries retained or distributed; integration is socket/CLI-only and installation stays with the user's official installer chain; recorded in `THIRD_PARTY_NOTICES.md` |
| `amosblomqvist/pi-config`, `pi-interactive-subagents`, `pi-observational-memory` | Design references only | No copied runtime source; clean-room posture recorded in `THIRD_PARTY_NOTICES.md`; any future code absorption requires explicit permission and notices updates |
