# AILI Compact（已废弃并从 0.2.0 运行时移除）

> 本文仅保留历史设计记录。0.2.0 不注册 AILI Compact 的命令、工具、provider projection、Session hook、compaction hook 或诊断；配置中的 `enabled: true` 也不会启用它。上下文压缩完全由 Pi 原生能力负责。源码暂留仓库，仅供未来重新设计，不属于受支持的发行能力。

AILI Compact 曾是 Pi Session 之上的实验性可逆上下文投影层。以下内容描述已废弃实现，不是当前使用说明。
...
配置从 global `~/.pi/agent/aili-compact.jsonc` 与 project `.pi/aili-compact.jsonc` 读取，project 覆盖 global；AILI 不创建或迁移这些文件。默认 `enabled=false`，显式设为 `true` 才启用实验性投影。关键 defaults：
...
- `@rosetears/aili-pi@0.2.0-preview.0` 没有实际 preview publication；exact local tarball/package discovery、installed-package 与 real-provider evidence 均为 Unverified/non-PASS，且不能变成 `latest` 或 stable release claim；
- second bounded official-Pi fixture attempt 为 `quality-rejected`，没有 controlled-production PASS artifact；historical sanitized record 对 `m000012`–`m000019` 记录 two hard facts / one missing。该观察不证明 root cause，也不证明 Pi incompatibility；
