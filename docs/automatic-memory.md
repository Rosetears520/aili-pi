...
The bridge reuses AILI's existing session MCP adapter. It creates no second MCP client and no local disk cache, durable outbox, SQLite/Markdown mirror, or fallback store. Fresh MemPalace 3.9.0 source evidence exposes 45 MCP tools; the required arguments used by AILI's four-tool allowlist (`mempalace_search`, `mempalace_check_duplicate`, `mempalace_add_drawer`, and `mempalace_diary_write`) remain compatible with the prior contract. The new `mempalace_search` arguments `candidate_strategy` and `cli_compatible` are optional and AILI does not send them. Extension load and session start do not execute a version probe. Only after the user initiates `/memory-auto authorize` and approves its interactive prompt does the runtime execute the no-shell, timeout/output-bounded `mempalace --version` probe. It rebinds the session port and arms durable operations only when the output identifies exact MemPalace `3.9.0`; an absent command, timeout, malformed output, or version mismatch leaves compatibility false and durable memory unarmed. The user-level environment inspected for this change reported exact `3.9.0`, but each session must obtain its own evidence. No live durable write is claimed by repository tests or documentation.
