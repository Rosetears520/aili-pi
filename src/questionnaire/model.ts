...
export function normalizeQuestions(input: readonly QuestionnaireQuestionInput[]): QuestionnaireQuestion[] {
	// AILI deviation from the absorbed PiCraft copy: no upper question cap —
	// four or fewer is recommended (the tool description says so), but a
	// genuinely wider clarification batch is accepted rather than truncated.
