...
export const WRITE_SCOPE_SCHEMA = Type.Object({
...
const ItemFields = {
  ...
  writeScope: Type.Optional(WRITE_SCOPE_SCHEMA),
...
export function normalizeWriteScope(value: unknown, label: string): TaskWriteScope {
...
export function roleCanMutate(role: RoleProfile): boolean {
  return role.tools.some((tool) => tool === "write" || tool === "edit")
    || role.capabilities.includes("repo.write");
}
...
function normalizeItem(
  ...
  const role = roleBySelector.get(selector)!;
  if (formalContext && roleCanMutate(role) && writeScope.paths.length === 0 && writeScope.resources.length === 0) {
    throw new Error(`${label}.formalContext requires a non-empty normalized writeScope for mutation-capable role '${selector}'`);
  }
