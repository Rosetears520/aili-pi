...
interface SubToolInfo {
  name: string;
  description: string;
  parameters: unknown;
  promptGuidelines?: readonly string[];
  sourceInfo?: SubSourceInfo;
}

interface SubSourceInfo {
  path: string;
  source: string;
  scope: string;
  origin: string;
...
interface SourceInfoBinding extends SubSourceInfo {
  identity: SubSourceInfo;
}

interface CanonicalSubReservation {
...
const reservations = new WeakMap<object, CanonicalSubReservation>();

function currentSub(pi: ExtensionAPI): { available: boolean; tool?: SubToolInfo } {
  try {
    return {
      available: true,
      tool: (pi.getAllTools() as SubToolInfo[]).find((candidate) => candidate.name === "sub"),
    };
  } catch {
...
function hasCanonicalDefinitionIdentity(tool: SubToolInfo | undefined, reservation: CanonicalSubReservation): boolean {
...
function validSourceInfo(sourceInfo: SubSourceInfo | undefined): sourceInfo is SubSourceInfo {
...
function bindSourceInfo(reservation: CanonicalSubReservation, sourceInfo: SubSourceInfo | undefined): boolean {
...
function matchesSourceInfo(sourceInfo: SubSourceInfo | undefined, binding: SourceInfoBinding): boolean {
...
function canonicalOwnerSourceInfo(pi: ExtensionAPI): SubSourceInfo | undefined {
  try {
    return pi.getCommands().find((command) => command.name === "aili-agent-model")?.sourceInfo as SubSourceInfo | undefined;
  } catch {
    return undefined;
  }
}

function isReservedDefinition(tool: SubToolInfo | undefined, reservation: CanonicalSubReservation): boolean {
 ...
 * reserve its exact
 * in-process definition and loader-owned source identity. Pi 0.84.4 exposes
 ...
 */
export function registerCanonicalAiliSubTool(pi: ExtensionAPI, definition: ToolDefinition): void {
