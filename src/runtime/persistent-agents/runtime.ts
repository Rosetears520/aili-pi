...
const SUB_PROMPT_GUIDELINES = [
  ...
  "Before first Herdr use (including sub on the Herdr backend), the Parent reads the installed herdr Skill and follows its environment/help prerequisites; consult current command help as needed, never guess syntax. Before each external CLI launch, judge startup readiness from the selected installed CLI's help/docs and relevant non-secret configuration: exact task working directory, workspace trust and unattended permission support. Reuse findings while the session, CLI, directory and relevant configuration are unchanged; inspect again when changed or contradicted by a failure. This is model judgment, not a vendor/version trust detector or a new runtime readiness gate. If Skill/help or required configuration evidence is unavailable, report the gap rather than assume readiness.",
  "External CLI policy: the runtime enables an existing allowlisted native YOLO/no-approval flag by default only when installed help supports it, independently of the parent Pi permission mode; unsupported stays yolo-unavailable, with no invented flag or silent permission-mode change. Native no-approval mode does not imply workspace trust. For an authorized external CLI task, the Parent may establish official trust for only its exact task working directory without another confirmation: prefer a supported temporary mechanism; use a narrowly scoped persistent official configuration only when necessary, preserving unrelated settings and reporting the change. Inspect the actual mechanism before writing; never trust a parent directory, an entire drive or all workspaces, change credentials, or bypass a tool denial. Unclear mechanisms, broader effects or conflicting restrictions require a focused user decision. Do not pass arbitrary runner flags through sub or manually launch a substitute child. This narrow startup policy grants no unrelated task, Git, publication or release authority.",
  ...
  "Lightweight TODO and Progress: tracked multi-action work, delegation, dependencies, blockers, cross-turn work, or an explicit request requires the Parent to maintain todo.md and progress.txt. Resolve the task and allowed root before listing observable actions and substantive execution. Simple Q&A or a single step without follow-up needs neither unless requested. Use the explicit user target, then project task convention, otherwise propose tasks/<task-slug>/ and obey placement approval; reuse one root for both files. If writing is forbidden or unavailable, give an in-conversation TODO and say it is not persisted; never bypass restrictions.",
  "Continuity updates: update TODO in place on start, completion, blocking, scope change, and before pause or closeout; normally highlight one current action, keeping genuine independent parallelism honest. Failed or uninspected Worker returns are not done. Keep unfinished items, blocker/next-decision and cancellation reasons. Reference accepted task IDs without mirroring tasks.md. Append Progress only for meaningful results, trade-offs, verification, blocker changes, and limits, never each tool call or raw logs. Resume reads TODO first, then recent or referenced Progress as needed; historical authorization does not renew.",
  "Continuity boundary: neither file has fixed grammar or a dispatch/settlement/completion format gate; notes never grant authority. Runtime Journal owns Agent/job/turn/settlement state. Preserve legacy formal-task-board.md as history, extracting only relevant current actions from current evidence; never parse, repair, replay, rename, delete, or automatically migrate it.",
  "Worker boundary: workers return evidence only; they never write the owning todo.md/progress.txt or legacy formal-task-board.md, or decide lifecycle phase, acceptance, or final verdict.",
];
