...
import { planEmergencyGc, planGenerationalGc, reconstructCompletedCompactionEpoch } from "./compaction.js";
import {
...
type SessionRuntime = {
  ...
  lastCheckpointOrigin: "deterministic" | "native" | "Unverified";
  lastHostWillRetry?: boolean;
...
export interface AiliCompactDiagnostics {
  ...
  contextUsageState: "observed" | "rebuilding" | "Unverified";
  lastCheckpointOrigin: SessionRuntime["lastCheckpointOrigin"];
  hostWillRetry?: boolean;
...
export function registerAiliCompact(pi: ExtensionAPI): void {
  ...
  pi.on("context", (event, ctx) => {
    ...
    const suffix = state.enabled && runtime.config.providerSuffix.enabled && lastAtom?.kind !== "remainder" && runtime.safePlan
      ? buildProviderSuffix({
        ...
        safeRanges: canRecommendSemanticAction(runtime) ? runtime.safePlan.ranges : [],
  ...
  pi.on("session_compact", (event, ctx) => {
    ...
    const epoch = reconstructCompletedCompactionEpoch({
      cancelled: false,
      compactionEntry: persistedIndex >= 0 ? entries[persistedIndex] : undefined,
      keptTailEntries: persistedIndex >= 0 ? entries.slice(persistedIndex + 1) : [],
    });
    let source = event.fromExtension ? "extension checkpoint" : "Pi native checkpoint";
    let observed = false;
    if (runtime && epoch) {
      const before = runtime.checkpoint.snapshot().tuple;
      const details = (entries[persistedIndex] as { details?: unknown } | undefined)?.details;
      const origin = event.fromExtension && isRecord(details) && isRecord(details.ailiCompact)
        && details.ailiCompact.kind === "major-gc-v3"
        ? "deterministic"
        : event.fromExtension ? "unverified" : "native";
      observed = runtime.checkpoint.observeEpoch(before, epoch.epochId, origin);
      if (!observed) source = "unverified checkpoint";
      if (!observed) {
        ctx.ui.setStatus("aili-compact", `AILI Compact WARN: completed ${source} epoch could not be adopted`);
        return;
      }
...
function doctorTelemetryFor(state: CompactState, runtime: SessionRuntime, view: V3RuntimeView | undefined = runtime.indexedView) {
  ...
  return buildCompactDoctorTelemetry({
    ...
    rebuildingUsage: runtime.rebuildingUsage,
...
export function diagnosticsFor(ctx: ExtensionContext, runtime?: SessionRuntime): AiliCompactDiagnostics {
  ...
  return {
    ...
    contextUsageState: doctor.contextUsageState,
    lastCheckpointOrigin: current.lastCheckpointOrigin,
    ...(current.lastHostWillRetry === undefined ? {} : { hostWillRetry: current.lastHostWillRetry }),
...
async function executeV3Compact(
  ...
  if (params.mode === "blocks") {
    ...
    try {
      const identity = buildQualityIdentityContext({
        entries: exactQualityIdentityEntries(entries, selected.children.map(({ block }) => block.anchorEntryId)),
  ...
  try {
    const identity = buildQualityIdentityContext({
      entries: exactQualityIdentityEntries(entries, range.orderedEntryIds),
      sessionId: view.state.sessionId,
      branchLeafId: view.state.branchLeafId,
      epochId: view.state.epochId,
      sessionPath,
...
function exactQualityIdentityEntries(
  entries: readonly SessionLikeEntry[],
  orderedEntryIds: readonly string[],
): SessionLikeEntry[] {
  const byId = new Map(entries.map((entry) => [entry.id, entry]));
  const seen = new Set<string>();
  return orderedEntryIds.map((entryId) => {
    if (seen.has(entryId)) throw new Error("duplicate quality identity entry");
    seen.add(entryId);
    const entry = byId.get(entryId);
    if (!entry || entry.type !== "message") throw new Error("missing quality identity message");
    return entry;
  });
}
...
function v3PromotionEpochEntries(entries: readonly SessionLikeEntry[], epochId: string): readonly SessionLikeEntry[] {
  if (epochId === "root") {
    const firstBoundary = entries.findIndex((entry) => entry.type === "compaction");
    return firstBoundary < 0 ? entries : entries.slice(0, firstBoundary);
  }
...
function canRecommendSemanticAction(runtime: SessionRuntime): boolean {
...
function hasValidPostCheckpointUsage(value: unknown): boolean {
  if (!isRecord(value)) return false;
  return [value.input, value.totalTokens].some((candidate) => typeof candidate === "number"
...
function recoveryStatus(runtime: SessionRuntime, state?: CompactState) {
  ...
  return {
    checkpointCoordinatorState: checkpoint.state,
    checkpointInFlight: checkpoint.inFlight,
    pressureStage: runtime.rebuildingUsage ? "Unverified" : runtime.pressure?.stage ?? "Unverified",
    headroomTokens: runtime.rebuildingUsage
      ? { source: "Unverified" }
      : runtime.pressure ? { value: runtime.pressure.headroomTokens, source: runtime.pressure.source } : { source: "Unverified" },
    contextUsageState: runtime.rebuildingUsage ? "rebuilding" : runtime.pressure ? "observed" : "Unverified",
    lastCheckpointOrigin: runtime.lastCheckpointOrigin,
    ...(runtime.lastHostWillRetry === undefined ? {} : { hostWillRetry: runtime.lastHostWillRetry }),
    deterministicCheckpointEligible: runtime.deterministicCheckpointEligible,
