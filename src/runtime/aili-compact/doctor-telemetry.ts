...
export function buildCompactDoctorTelemetry(input: CompactDoctorTelemetryInput) {
  ...
  return {
    pressureStage: input.rebuildingUsage ? "Unverified" as const : input.pressure?.stage ?? "Unverified" as const,
    headroomTokens: !input.rebuildingUsage && input.pressure
      ? { value: input.pressure.headroomTokens, source: input.pressure.source }
      : { source: "Unverified" as const },
    contextUsageState: input.rebuildingUsage ? "rebuilding" as const : input.pressure ? "observed" as const : "Unverified" as const,
    components: {
