...
export function planV3BlockMutation(
  ...
  for (let index = 1; index < children.length; index += 1) {
    if (children[index]!.block.firstLeafOrdinal <= children[index - 1]!.block.lastLeafOrdinal) {
      return failure("non-contiguous-children", "Selected child coverage is reordered or overlapping.", "$.blockRefs", context);
    }
  }
