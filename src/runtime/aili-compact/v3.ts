...
export const V3_LIMITS = {
  ...
  minChildBlocks: 2,
  maxChildBlocks: 16,
  maxTransparentPromotionGaps: MAX_TRANSPARENT_PROMOTION_GAPS,
  maxTransparentPromotionGapMessages: MAX_TRANSPARENT_PROMOTION_GAP_MESSAGES,
  maxRootBlocks: 16,
...
export interface V3BlockSource {
  ...
  transparentGaps?: TransparentPromotionGapV1[];
}
...
function parseSemanticSource(value: unknown): V3Result<V3SemanticSource> {
  ...
  if (value.kind === "blocks") {
    ...
    if (!childBlockIds.ok) return childBlockIds;
    const transparentGaps = parseTransparentPromotionGaps(value.transparentGaps, path);
    if (!transparentGaps.ok) return transparentGaps;
    return ok({
      kind: "blocks",
      childBlockIds: childBlockIds.value,
      ...(transparentGaps.value.length > 0 ? { transparentGaps: transparentGaps.value } : {}),
    });
  }
...
function validateBlockStructure(
  ...
  for (let index = 1; index < exact.length; index += 1) {
    if (exact[index]!.firstLeafOrdinal <= exact[index - 1]!.lastLeafOrdinal) return fail("non-contiguous-children", `${path}.source.childBlockIds`, undefined);
  }
  if (structuralPromotionGapError(exact, block.source)) return fail("invalid-promotion-gap", `${path}.source.transparentGaps`, undefined);
...
function cloneSource(source: V3SemanticSource): V3SemanticSource {
  return source.kind === "messages"
    ...
    : {
      ...source,
      childBlockIds: [...source.childBlockIds],
      ...(source.transparentGaps ? { transparentGaps: source.transparentGaps.map((proof) => ({ ...proof })) } : {}),
    };
}

function parseTransparentPromotionGaps(value: unknown, sourcePath: string): V3Result<TransparentPromotionGapV1[]> {
  if (value === undefined) return ok([]);
  const path = `${sourcePath}.transparentGaps`;
  if (!Array.isArray(value) || value.length > V3_LIMITS.maxTransparentPromotionGaps) return fail("invalid-promotion-gap", path, undefined);
  const proofs: TransparentPromotionGapV1[] = [];
  for (const [index, candidate] of value.entries()) {
    const itemPath = `${path}.${index}`;
    if (!isRecord(candidate)) return fail("invalid-promotion-gap", itemPath, undefined);
    const extra = unknownField(candidate, TRANSPARENT_GAP_KEYS, itemPath, "invalid-promotion-gap");
    if (extra) return extra;
    if (candidate.version !== 1
      || !boundedString(candidate.leftChildBlockId, V3_LIMITS.maxIdentifierChars)
      || !boundedString(candidate.rightChildBlockId, V3_LIMITS.maxIdentifierChars)
      || !boundedString(candidate.leftLeafEntryId, V3_LIMITS.maxIdentifierChars)
    ...
    proofs.push({
      version: 1,
      leftChildBlockId: candidate.leftChildBlockId,
      rightChildBlockId: candidate.rightChildBlockId,
      leftLeafEntryId: candidate.leftLeafEntryId,
...
function verifyPromotionGapProofs(
  blocks: ReadonlyMap<string, V3SemanticBlock>,
...
function structuralPromotionGapError(children: readonly V3SemanticBlock[], source: V3BlockSource): boolean {
  const expectedPairs = children.flatMap((child, index) => index > 0 && child.firstLeafOrdinal !== children[index - 1]!.lastLeafOrdinal + 1
    ? [[children[index - 1]!.blockId, child.blockId] as const]
    : []);
  const proofs = source.transparentGaps ?? [];
  return proofs.length !== expectedPairs.length || proofs.some((proof, index) => {
    const pair = expectedPairs[index];
    return !pair || proof.leftChildBlockId !== pair[0] || proof.rightChildBlockId !== pair[1];
  });
}

function proofsEqual(left: readonly TransparentPromotionGapV1[], right: readonly TransparentPromotionGapV1[]): boolean {
  return left.length === right.length && left.every((proof, index) => {
    const expected = right[index];
    return expected !== undefined
      && proof.version === expected.version
      && proof.leftChildBlockId === expected.leftChildBlockId
      && proof.rightChildBlockId === expected.rightChildBlockId
