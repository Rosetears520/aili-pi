...
import { fetchImageAttachment, imageMimeFor, quotePathIfNeeded } from "@/lib/file-attach";
...
interface Props {
  ...
  onSwitchPermMode?: (mode: string) => void;
  /** Executes a slash command without touching the composer draft. */
...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput({
  ...
  const attachImage = useCallback((image: { name: string; data: string; mimeType: string }) => {
    setAttachedImages((prev) => {
      if (prev.length >= MAX_ATTACHED_IMAGES) return prev;
      const next = [...prev, { data: image.data, mimeType: image.mimeType, previewUrl: `data:${image.mimeType};base64,${image.data}` }];
  ...
  useEffect(() => {
    const onOpenBtw = () => setBtwOpen(true);
    window.addEventListener("aili:btw:open", onOpenBtw);
    return () => window.removeEventListener("aili:btw:open", onOpenBtw);
  }, []);
  ...
  const insertAtCursor = useCallback((text: string, options?: { fromPaste?: boolean }) => {
    ...
    const sep = !options?.fromPaste && before.length > 0 && !before.endsWith(" ") ? " " : "";
  ...
  // Non-image files arriving via paste/drop carry no local path the browser
  // will reveal; store them in the server scratch directory and insert paths.
  const uploadAndInsertPaths = useCallback(async (files: File[]) => {
    for (const file of files) {
      try {
        const scope = permSessionId ? `&session=${encodeURIComponent(permSessionId)}` : "";
        const response = await fetch(`/api/aili/upload?name=${encodeURIComponent(file.name)}${scope}`, {
          method: "POST",
          headers: { "Content-Type": file.type || "application/octet-stream" },
          body: file,
        });
        const body = await response.json().catch(() => ({}));
        if (!response.ok || typeof body?.path !== "string") throw new Error(body?.error ?? `HTTP ${response.status}`);
        insertAtCursor(`${quotePathIfNeeded(body.path)} `);
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        setUploadNotice(`${file.name}: ${message}`);
        setTimeout(() => setUploadNotice((current) => current === `${file.name}: ${message}` ? null : current), 8000);
        console.warn(`[aili] could not store pasted file ${file.name}`, error);
      }
    }
  }, [insertAtCursor, permSessionId]);

  // Plus control: the Windows-native file dialog (WSL interop) is the primary
  // path; the in-web picker opens as the fallback when it is unavailable.
  const openPlus = useCallback(async () => {
    if (plusBusy || isStreaming) return;
    setPlusBusy(true);
    let fellBack = false;
    try {
      const response = await fetch("/api/aili/native-file-dialog", {
        method: "POST",
      ...
      if (response.status === 501 || response.status === 404) {
        ...
        const paths: string[] = Array.isArray(body?.paths) ? body.paths.filter((item: unknown): item is string => typeof item === "string") : [];
        for (const absolutePath of paths) {
          if (imageMimeFor(absolutePath)) {
            const fetched = await fetchImageAttachment(absolutePath);
            if ("error" in fetched) {
              if (fetched.error.includes("exceeds")) {
                // A real path exists — reference it instead of dropping the file.
                insertAtCursor(`${quotePathIfNeeded(absolutePath)} `);
              } else {
                console.warn(`[aili] could not attach ${absolutePath}: ${fetched.error}`);
              }
              continue;
            }
            attachImage(fetched);
        ...
        console.warn(`[aili] native file dialog failed (HTTP ${response.status})`);
      }
    } catch (error) {
      console.warn("[aili] native file dialog unavailable; opening the web picker", error);
      setPickerOpen(true);
      fellBack = true;
    } finally {
      setPlusBusy(false);
    ...
    void fellBack;
  }, [plusBusy, isStreaming, cwd, attachImage, insertAtCursor]);
  useImperativeHandle(ref, () => ({
    ...
    addFiles(files: File[]) {
      void uploadAndInsertPaths(files);
    },
  }));
  ...
  }, [isStreaming, onBuiltinCommand, onSend, onAudioUnlock]);
  ...
  const handlePaste = useCallback((e: React.ClipboardEvent) => {
    ...
    const images = files.filter((file) => file.type.startsWith("image/") && file.size <= MAX_ATTACHED_IMAGE_BYTES);
    // Oversized images and non-image files become scratch-stored path
    // references instead of being silently dropped or ignored.
    const asPaths = files.filter((file) => !images.includes(file));
    if (images.length) {
      e.preventDefault();
    ...
    if (asPaths.length) {
      e.preventDefault();
      void uploadAndInsertPaths(asPaths);
    }
    if (files.length > 0) return;
    // Whole-paste WSL normalization: convert only when the entire pasted text
    // is exactly one Windows path; prose that mentions a path is untouched.
    const pastedText = e.clipboardData?.getData("text/plain") ?? "";
    if (!pastedText) return;
    void fetchWslEnv().then((env) => {
      const normalized = normalizeWindowsPath(pastedText, env);
      if (normalized === undefined) return;
      e.preventDefault();
      insertAtCursor(`${normalized} `, { fromPaste: true });
    });
  }, [processImageFiles, uploadAndInsertPaths, insertAtCursor]);
  ...
  return (
    ...
    >
      ...
      <div style={{ maxWidth: 820, margin: "0 auto" }}>
        ...
        <div style={{ position: "relative", minWidth: 0 }}>
          ...
          <div
            ...
            style={{
              ...
              flexDirection: "column",
              background: "var(--bg)",
              border: `1px solid ${composerFocused
              ...
              boxShadow: composerFocused
                ? "0 0 0 3px color-mix(in srgb, var(--accent) 14%, transparent), 0 8px 24px -12px rgba(15,23,42,0.10)"
        ...
        {uploadNotice && (
          <div className="aili-upload-notice" role="status">{uploadNotice}</div>
        )}

        {/* Bash mode status label */}
        {bashMode && (
          <div className="text-xs px-2 py-1" style={{ color: bashExcluded ? "var(--text-muted)" : "var(--accent)", marginTop: 2 }}>
             {t("chat.shell")} · {bashExcluded ? t("chat.outputLocal") : t("chat.outputModel")}
          </div>
        )}

        {/* Bottom control row: left cluster | spacer | right cluster */}
        <div style={{
          marginTop: 6,
          ...
          {/* LEFT: attach + mode chip + BTW (idle) or steer/follow-up (streaming) */}
          <div style={{ flex: isMobile ? "1 1 auto" : "0 0 auto", minWidth: 0, display: "flex", alignItems: "center", gap: 2 }}>
            ...
            <button
              ...
              disabled={isStreaming || plusBusy}
              title={plusBusy ? "Waiting for the native file dialog…" : "Insert file — native Windows dialog (images attach, other files insert their path)"}
              aria-label="Insert file"
              style={{
                flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center",
                width: 32, height: 32, padding: 0,
                background: pickerOpen ? "var(--bg-hover)" : "none",
                border: "none",
                borderRadius: 9,
                color: pickerOpen ? "var(--accent)" : attachedImages.length ? "var(--accent)" : "var(--text-muted)",
                cursor: isStreaming || plusBusy ? "not-allowed" : "pointer",
                opacity: isStreaming ? 0.5 : 1,
                transition: "background 0.12s, color 0.12s",
              }}
            >
              {plusBusy ? (
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" style={{ animation: "spin 0.8s linear infinite" }} aria-hidden="true">
            ...
            {!isStreaming && (
              <>
                <button
                  ...
                  title="BTW side thread (floating)"
                  aria-label="Open BTW side thread"
                  style={{
                    flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center",
                    width: 32, height: 32, padding: 0,
                    background: btwOpen ? "var(--bg-hover)" : "none",
                    border: "none",
                    borderRadius: 9,
                    color: btwOpen ? "var(--accent)" : "var(--text-muted)",
                ...
                >
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M21 11.5a8.38 8.38 0 0 1-9 8.4 8.5 8.5 0 0 1-3.4-.7L3 21l1.9-5.1a8.38 8.38 0 0 1-.9-3.9 8.5 8.5 0 0 1 8.5-8.5 8.38 8.38 0 0 1 8.5 8Z" />
          ...
          <div ref={controlsMenuRef} style={{
            ...
            {isStreaming ? (
              <button
                className="aili-send aili-send--stop"
                onClick={onAbort}
          ...
          {btwOpen && (
            <AiliBtwDialog
              ...
              currentModel={model ? { provider: model.provider, modelId: model.modelId } : undefined}
