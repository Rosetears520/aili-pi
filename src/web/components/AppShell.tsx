...
export function AppShell() {
  ...
  useEffect(() => {
    window.addEventListener("aili:changes:open", openChangesPage);
        ...
        {!mobile && (
          <>
            <button
              ...
              onMouseLeave={(event) => { event.currentTarget.style.color = "var(--text-muted)"; }}
              aria-label="Changes"
              title="Working tree and upstream changes"
            >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "0 8px", borderRight: "1px solid var(--border)", height: "100%" }}>
              <button
                type="button"
                ...
                onMouseLeave={(event) => { event.currentTarget.style.background = "none"; event.currentTarget.style.color = soundEnabled ? "var(--text-muted)" : "var(--text-dim)"; }}
              >
                {soundEnabled ? (
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
  ...
  return (
        ...
        <div style={{ flex: 1, overflow: "hidden", position: "relative" }}>
          {showChat ? (
            <ChatWindow
              ...
              onAiliSurfaceChange={handleAiliSurfaceChange}
    ...
    {terminalOpen && (activeCwd ?? selectedSession?.cwd ?? newSessionCwd) && (
      <TerminalPanel cwd={(activeCwd ?? selectedSession?.cwd ?? newSessionCwd)!} onClose={() => setTerminalOpen(false)} />
    )}
