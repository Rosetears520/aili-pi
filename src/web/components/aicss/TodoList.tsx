...
export interface TodoListProps {
  /** Task labels in order. Omit to run the original aicss self-playing demo. */
  items?: string[];
  /** Index of the in-progress task: -1 = not started, 0..n-1 = working on that
   * task, >= items.length = all done. Ignored (demo auto-plays) when `items`
   * is omitted. */
  current?: number;
  /** Heading text for the collapsible header. */
  title?: string;
  /** Start collapsed instead of expanded. */
  defaultCollapsed?: boolean;
}

export function TodoList({ items, current: currentProp, title, defaultCollapsed = false }: TodoListProps) {
  const demo = items === undefined;
  const labels = items ?? LABELS;
  const [collapsed, setCollapsed] = useState(defaultCollapsed);
  // -1 = not started (plan shown), 0..n-1 = working on that task, n = all done
  const [current, setCurrent] = useState(currentProp ?? -1);
  const n = labels.length;

  useEffect(() => {
    if (!demo) {
      setCurrent(currentProp ?? -1);
      return;
    }
  ...
  return (
    <div className={styles.todo}>
      ...
      >
        ...
        <span className={styles.todoTitle}>{title ?? "To-dos"}</span>
