...
export function ChatWindow({ session, sessionRunning, newSessionCwd, newSessionDraftKey, onAgentEnd, onAttentionNeeded, onSessionCreated, onSessionForked, modelsRefreshKey, chatInputRef, onBranchDataChange, onSystemPromptChange, onSystemPromptLoaderChange, onSessionStatsChange, onSessionStatsPanelOpen, onContextUsageChange, onOpenFile, onAiliSurfaceChange, soundEnabled = true, onSoundToggle, playDoneSound = () => {}, unlockAudio }: Props) {
  ...
  // Self-issued /perm commands switch modes like the TUI shortcut: they run
  // through the same extension pathway but are not echoed as user messages.
  const permEchoRef = useRef(new Map<string, number>());
  const runAiliCommand = useCallback((command: string) => {
    if (command.startsWith("/perm ")) permEchoRef.current.set(command, Date.now());
    void handleSend(command);
  }, [handleSend]);

  // Direct-feel switching: invoke the session's /perm handler without the
  // prompt pipeline (no journaled user message, no command-run phase). Falls
  // back to the command path if direct execution is unavailable.
  // Permission switching rides the same session RPC channel as model and
  ...
  useEffect(() => {
    onAiliSurfaceChange?.({
      ...
      statuses: extensionStatuses.map((item) => ({ key: item.key, text: item.text })),
    });
  }, [extensionStatuses, session?.cwd, newSessionCwd, onAiliSurfaceChange]);

  // opencode-style configurable shortcuts; typing inputs are never hijacked.
  useEffect(() => {
    let keybinds: WebKeybinds = {};
    let cancelled = false;
    void fetchWebKeybinds().then((loaded) => {
      if (!cancelled) keybinds = loaded;
    });
    const onKeyDown = (event: KeyboardEvent) => {
      const action = actionForBinding(keybinds, eventToBinding(event));
      if (!action) return;
      // TUI parity: mode cycling works even while typing in the composer,
      // like the TUI's alt+m; other actions stay input-safe.
      if (action !== "mode.cycle" && isTextInput(event.target)) return;
      event.preventDefault();
      if (action === "changes.open") {
        window.dispatchEvent(new CustomEvent("aili:changes:open"));
        return;
      }
      if (action === "panel.btw") {
        window.dispatchEvent(new CustomEvent("aili:btw:open"));
        return;
      }
      if (action === "mode.cycle") {
        window.dispatchEvent(new CustomEvent("aili:mode:cycle"));
        return;
      }
    };
  ...
  const onDrop = useCallback((files: File[]) => {
    ...
    if (images.length) handle.addImages(images);
    if (asPaths.length) handle.addFiles(asPaths);
  }, [chatInputRef]);
  ...
  const visibleMessages = messages.filter((m) => {
    ...
    if (m.role === "user" && typeof m.content === "string" && permEchoRef.current.size > 0) {
      const now = Date.now();
      for (const [echo, at] of permEchoRef.current) {
        if (now - at > 30_000) permEchoRef.current.delete(echo);
      }
      const echoAt = permEchoRef.current.get(m.content);
      if (echoAt !== undefined) {
        permEchoRef.current.delete(m.content);
        return false;
      }
    }
    return true;
