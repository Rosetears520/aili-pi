...
export class AgentSessionWrapper {
  ...
  async send(command: Record<string, unknown>): Promise<unknown> {
    ...
    switch (type) {
      ...
      case "set_perm_mode": {
        const mode = String((command as { mode?: unknown }).mode ?? "").trim().toLowerCase();
        if (!/^[a-z0-9][a-z0-9-]*$/.test(mode)) throw new Error("Invalid permission mode");
        // Same in-process handler the TUI shortcut reaches; no prompt pipeline,
        // no journaled user message, no command-run phase.
        const direct = this.inner as unknown as { _tryExecuteExtensionCommand?(text: string): Promise<boolean> };
        if (typeof direct._tryExecuteExtensionCommand !== "function") {
          throw new Error("Direct command execution unavailable");
        }
        const executed = await direct._tryExecuteExtensionCommand(`/perm ${mode}`);
        if (!executed) throw new Error("perm command not registered");
  ...
  private createExtensionUiContext(): ExtensionUiContextLike {
    return {
      ...
      // AILI Unified User Interaction: one full questionnaire card. No timeout
      // is ever set — the user is never auto-answered; dismissal or abort
      // resolves as an explicitly cancelled result with empty answers.
      questionnaire: (questions: QuestionnaireQuestion[], signal?: AbortSignal) => {
        const cancelled = (): QuestionnaireResult => ({
          questions,
          answers: createAnswers(questions),
          cancelled: true,
        });
        return this.requestExtensionUi<QuestionnaireResult>(
          { method: "questionnaire", questions },
          cancelled(),
          (response) => "answers" in response
            ? { questions, answers: response.answers, cancelled: false }
            : cancelled(),
          undefined,
          signal,
        );
