...
describe("Pi 0.84.4 task winner collision policy", () => {
  it.each([
    "canonical-first",
    "collision-first",
    ...
    const canonical: ExtensionFactory = async (pi) => {
      registerCanonicalAiliSubTool(pi, taskDefinition("canonical", () => { calls.canonical += 1; }));
...
function canonicalExtension(invoked: () => void, genericInvoked: () => void): ExtensionFactory {
  return async (pi) => {
    registerCanonicalAiliSubTool(pi, taskDefinition("canonical", invoked));
