...
describe("AILI Compact runtime", () => {
  ...
  it("keeps dynamic planning out of the system prompt at checkpoint pressure", () => {
    const runtime = harness();
    const entries = v3CompressibleEntries();
    const ctx = context(entries, { tokens: 70_000, contextWindow: 100_000 });
    runtime.handlers.get("session_start")!({ type: "session_start" }, ctx);
  ...
  it("omits suffix actions and range references when checkpoint pressure disallows semantic work", () => {
    const runtime = harness();
    const entries = v3CompressibleEntries();
    const ctx = context(entries, { tokens: 90_000, contextWindow: 100_000 });
    runtime.handlers.get("session_start")!({ type: "session_start" }, ctx);
    const projected = runtime.handlers.get("context")!({ type: "context", messages: providerMessages(entries) }, ctx);
    expect(projected.messages.some((message: any) => message.customType === "aili-compact-provider-suffix")).toBe(false);
  });
  ...
  it("returns exact undefined for legacy overflow coverage without accepted current-layer proof", () => {
    const runtime = harness();
  ...
  it("preserves native fallback for legacy coverage without accepted current-layer proof", () => {
    const runtime = harness();
    ...
    expect(runtime.handlers.get("session_before_compact")!(beforeCompactEvent(safeEntries, "threshold", "user"), safe)).toBeUndefined();
  ...
  it("accepts only a persisted new epoch and masks usage as rebuilding until an assistant reports usage", async () => {
    const runtime = harness();
    const entries: any[] = [{ id: "old", type: "message", message: { role: "user", content: "old context" } }];
    const ctx = context(entries, { tokens: 90_000, contextWindow: 100_000 });
    runtime.handlers.get("session_start")!({ type: "session_start" }, ctx);
    const persisted = { id: "native-epoch", type: "compaction", summary: "native summary" };
    entries.push(persisted);
    runtime.handlers.get("session_compact")!({ type: "session_compact", compactionEntry: persisted, fromExtension: false, willRetry: true }, ctx);

    const rebuilding = await statusSnapshot(runtime, entries, ctx);
    expect(rebuilding).toMatchObject({
      contextUsageState: "rebuilding",
      lastCheckpointOrigin: "native",
      hostWillRetry: true,
      headroomTokens: { source: "Unverified" },
    });

    runtime.handlers.get("message_end")!({ type: "message_end", message: {
      role: "assistant", content: "new epoch response", usage: { input: 10, output: 2, cacheRead: 0, cacheWrite: 0 }, stopReason: "stop",
    } }, ctx);
    const observed = await statusSnapshot(runtime, entries, ctx);
    expect(observed.contextUsageState).toBe("observed");
  });
