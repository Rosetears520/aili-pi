...
describe("AILI Compact copied-session forward and rollback migration", () => {
  ...
  it("proves fork/epoch isolation, source search and IDs, suffix non-persistence, and rollback opening", () => {
    ...
    expect(scenario.suffix).toEqual({ injected: true, persisted: false });
...
async function runMigrationScenario(root: string): Promise<MigrationScenario> {
  ...
  requireCondition(suffixInjected && !suffixPersisted,
    `provider suffix migration evidence is incomplete: ${cleanContextStatus}`);
  ...
  const matrix: MatrixRow[] = [
    ...
    { id: "provider-suffix-non-persistence", status: "PASS", proof: "production context hook injected suffix while Session bytes excluded it" },
    ...
    { id: "separately-installed-v0.1.14-binary", status: "Unverified", proof: "no separately installed historical binary is available through repository public APIs" },
  ];
