...
import { AILI_COMPACT_ENTRY, sourceDigest, type SessionLikeEntry } from "../../src/runtime/aili-compact/contracts.js";
...
type LineagePlan =
  ...
  | { kind: "compact"; tier: "T1" | "T2" | "T3" | "T3-restill"; childCount?: number; summary: string; staleCatalog?: boolean; deriveSummaryFromExactRange?: boolean };
...
class LineageProvider {
  ...
  readonly streamSimple = (selected: Model<any>, context: Context, _options?: SimpleStreamOptions) => {
    ...
    const status = latestToolJson(context, "aili_compact_status");
...
describe("AILI Compact production AgentSession integration", () => {
  ...
  it("publishes full controlled-production lineage, suffix, rejection, native retry, and continued-work evidence", async () => {
    ...
    const anchors = Array.from({ length: 16 }, (_, index) => `L${String(index + 1).padStart(3, "0")}`);
    ...
    for (const [index, anchor] of anchors.entries()) {
      await runLineagePlan(session, provider, { kind: "text", text: "SOURCE_ACCEPTED" },
        `Fixture source must retain \`${anchor}\` ${"\u0002".repeat(80_000)}`);
      ...
      await runLineagePlan(session, provider, {
        kind: "compact",
        tier: "T1",
        summary: `${compactSummarySurface([anchor], "\u0001", 7_580)} SOURCE_ACCEPTED`,
      ...
      if (next.length !== before + 1) {
        ...
        throw new Error(`T1 ${index + 1} rejected: ${textOf(rejected?.message?.content).slice(0, 2_000)}`);
      }
      expect(next, `T1 ${index + 1}`).toHaveLength(before + 1);
      accepted.push(next.at(-1)!);
    }

    const t2AnchorGroups = chunk(anchors, 4);
    for (const [index, group] of t2AnchorGroups.entries()) {
      ...
      await runLineagePlan(session, provider, {
        ...
        tier: "T2",
        childCount: 4,
        summary: compactSummarySurface(group, "界", 8_000),
    ...
    const t3AnchorGroups = chunk(anchors, 8);
    ...
    await runLineagePlan(session, provider, {
      ...
      tier: "T3-restill",
      childCount: 2,
      summary: compactSummarySurface(anchors, "界", 1_000),
    }, "create controlled T3 restill");
    ...
    expect(lineage.counts).toEqual({ T1: 16, T2: 4, T3: 2, T3Restill: 1, total: 23 });
    expect(lineage.transactions).toHaveLength(23);
    expect(new Set(lineage.transactions.map((row) => row.transactionId)).size).toBe(23);
    expect(new Set(lineage.transactions.map((row) => row.sha256)).size).toBe(23);
    expect(accepted).toHaveLength(23);
    expect(provider.toolArguments.filter(({ name }) => name === "aili_compact")).toHaveLength(23);
    ...
    const artifact = {
      ...
      rows: [
        ...
        { id: "controlled-rejections", status: "PASS", rejectionCount: 2, transactionsAdded: 0 },
    ...
    writeFileSync(compactArtifactPath, `${JSON.stringify(artifact, null, 2)}\n`);
  }, 300_000);
});
...
function compactArguments(
  ...
  const selectedCatalog = plan.staleCatalog && catalogId.length === 64
    ...
    : catalogId;
  if (plan.tier === "T1") {
    const range = status.references?.safeRanges?.[0];
    if (range) {
      const summary = plan.deriveSummaryFromExactRange ? exactT1Summary(status, range, manager, plan.summary) : plan.summary;
      return {
  ...
  const sourceTier = plan.tier === "T2" ? "T1" : plan.tier === "T3" ? "T2" : "T3";
  const group = (status.references?.lifecycle?.structuralPromotionGroups ?? [])
    .find((candidate: JsonRecord) => candidate.sourceTier === sourceTier && candidate.blockRefs?.length >= (plan.childCount ?? 2));
  if (!group) return undefined;
  return {
    mode: "blocks", catalogId: selectedCatalog, topic: `controlled ${plan.tier} lineage`,
    blockRefs: group.blockRefs.slice(0, plan.childCount ?? 2), summary: plan.summary, summaryMaxChars: 10_000,
  };
}

function exactT1Summary(status: JsonRecord, range: JsonRecord, manager: SessionManager, fallbackSummary: string): string {
  const entries = manager.getBranch() as unknown as SessionLikeEntry[];
  ...
  const orderedRefs = Array.isArray(range.orderedRefs) && range.orderedRefs.every((ref: unknown) => typeof ref === "string")
    ...
    : [];
  if (status.references?.catalogId !== view.catalog.catalogId) throw new Error("fixture-catalog-id-mismatch");
  if (range.catalogId !== view.catalog.catalogId) throw new Error("fixture-range-catalog-id-mismatch");
  ...
  for (const message of view.mutationCatalog.messageRefs) {
    if (entryIdByRef.has(message.ref)) throw new Error("fixture-duplicate-durable-ref");
    entryIdByRef.set(message.ref, message.entryId);
  }
  ...
  const orderedEntryIds = orderedRefs.map((ref) => {
    ...
    if (!entryId) throw new Error("fixture-durable-ref-missing");
    if (entry?.type !== "message") throw new Error("fixture-source-entry-not-message");
    return entryId;
  });
  ...
  const anchors = [...new Set(frozen.facts.flatMap((fact) => fact.eligibility === "eligible"
    && qualityRequirement("T1", fact.class, fact.current, fact.releaseRelevant) === "hard" ? fact.anchors : []))];
  if (anchors.length === 0) return fallbackSummary;
  if (anchors.some((anchor) => anchor !== "SOURCE_ACCEPTED" && !/^L\d{3}$/.test(anchor))) {
    throw new Error("fixture-hard-anchor-unexpected");
  }
  return compactSummarySurface(anchors, "\u0001", 7_580);
}
...
function compactSummarySurface(anchors: readonly string[], fill: string, targetLength = 9_500): string {
