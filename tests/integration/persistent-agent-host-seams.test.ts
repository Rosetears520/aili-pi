...
describe("official Pi 0.84.4 persistent Agent host seams", () => {
  ...
  it("binds host seam evidence to the installed and declared Pi 0.84.4 baseline", async () => {
    ...
    expect(projectPackage.devDependencies["@earendil-works/pi-coding-agent"]).toBe("0.84.4");
    expect(installedPackage.version).toBe("0.84.4");
  });
});
