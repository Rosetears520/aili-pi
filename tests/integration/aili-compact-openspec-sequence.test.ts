...
const sourceRoots = [
  ...
  join(root, "openspec", "changes", "replace-pi-native-fallback-with-aili-emergency-checkpoint"),
  join(root, "openspec", "changes", "reconcile-aili-compact-release-lineage"),
];

describe("AILI Compact sequential OpenSpec materialization", () => {
  it("materializes and strict-validates all five stages through release-lineage reconciliation", async () => {
    ...
    expect(report).toMatchObject({
      ...
      releaseOrder: [
        ...
        "reconcile-aili-compact-release-lineage",
      ],
      scratch: { parent: ".tmp", prefix: scratchPrefix, cleaned: true },
      sourceWorkspaceMutated: false,
    });

    const [base, fix, redesign, emergencyCheckpoint, reconciliation] = report.stages;
    ...
    expect(emergencyCheckpoint).toMatchObject({
      stage: "base-plus-fix-plus-redesign-plus-emergency-checkpoint",
      changeId: "replace-pi-native-fallback-with-aili-emergency-checkpoint",
      status: "PASS",
      deltaPreflight: { status: "PASS", issues: [] },
      archive: { status: "PASS", exitCode: 0, semanticSuccess: true },
      materialization: { status: "PASS", expectedApplied: true, differences: [] },
      strictValidation: { status: "PASS", summary: { items: 6, passed: 6, failed: 0 } },
    });
    expect(reconciliation).toMatchObject({
      stage: "base-plus-fix-plus-redesign-plus-emergency-checkpoint-plus-release-lineage-reconciliation",
      changeId: "reconcile-aili-compact-release-lineage",
      status: "PASS",
      deltaPreflight: { status: "PASS", issues: [] },
      archive: { status: "PASS", exitCode: 0, semanticSuccess: true },
      materialization: { status: "PASS", expectedApplied: true, differences: [] },
      strictValidation: { status: "PASS", summary: { items: 7, passed: 7, failed: 0 } },
    });
    const reconciliationRecovery = reconciliation.materialization.operations
      .find((operation: any) => operation.capability === "aili-compact-checkpoint-recovery");
    expect(reconciliationRecovery).toEqual({
      capability: "aili-compact-checkpoint-recovery",
      added: ["P0 recovery inheritance is version-neutral"],
      modified: ["Release evidence covers fake, production-entry, and live recovery"],
      removed: [],
    });
  ...
  it("marks deterministic heading-only fallback Unverified instead of claiming merged-spec PASS", async () => {
    ...
    expect(report.stages.map((stage: any) => stage.status)).toEqual(["Unverified", "Unverified", "Unverified", "Unverified", "Unverified"]);
    ...
    expect(report.stages[4].deltaPreflight.issues).toEqual([]);
    expect(await ownedScratchNames()).toEqual(beforeScratch);
  }, 30_000);
});
