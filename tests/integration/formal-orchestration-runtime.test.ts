...
import { appendFormalResultContract, parseCanonicalFormalResult } from "../../src/runtime/persistent-agents/task-coordinator.js";
import { assembleChildPrompt } from "../../src/runtime/persistent-agents/policy.js";
...
function continuationAudit(writeScope = { paths: ["src"] as string[], resources: [] as string[] }, packageId = "P-01") {
  return {
    packageId,
...
function observation(id = "P-01", overrides: Partial<FormalRuntimeReconciliationObservation> = {}): FormalRuntimeReconciliationObservation {
  ...
  return {
    ...
    formalContinuationIdentity: continuationAudit({ paths: ["src"], resources: [] }, id),
...
describe("formal task Runtime allocation and protection identity", () => {
  it("assembles the real generated role with the formal override and parses output derived from that exact prompt", async () => {
    ...
    const assembled = assembleChildPrompt({
      runtimeEnvelope: "Official Pi formal fixture.",
      role,
      task: appendFormalResultContract("Implement the bounded fixture.", { packageId: "P-01", roleId: role.selector }),
      cwd: ".",
      workspace: { mode: "shared", root: "." },
    });
    expect(assembled.initialMessage).toContain("This formal assignment overrides the role's ordinary JSON output contract.");
    const marker = assembled.initialMessage.indexOf("CANONICAL RESULT:\n");
    expect(marker).toBeGreaterThan(0);
    const envelopeEnd = assembled.initialMessage.indexOf("\n\n## Current working directory", marker);
    const envelope = assembled.initialMessage.slice(marker, envelopeEnd).split("\n").map((line) => {
      if (line === "CANONICAL RESULT:") return line;
      const separator = line.indexOf(":");
      const key = line.slice(0, separator);
      if (key === "package_id" || key === "role_id") return line;
      if (key === "status") return "status: completed";
      if (key === "evidence") return "evidence: artifact:prompt-derived/P-01";
      if (key === "verification") return "verification: verification:prompt-derived/P-01";
      return `${key}: fixture-${key}`;
    }).join("\n");
    expect(parseCanonicalFormalResult(envelope, { packageId: "P-01", roleId: role.selector })).toMatchObject({ ok: true, value: { status: "completed" } });
  });
  ...
  it("fails closed for every non-canonical formal result and persists only exact completed/partial as completed", async () => {
    ...
    for (const name of Object.keys(outputs)) {
      const response = await runtimeInstance.task.submit({
        task: name,
  ...
  it("rejects malformed/non-v1/non-exact roots before any child allocation and persists the valid derived deny set", async () => {
    ...
    await expect(runtimeInstance.task.submit({
    ...
    await expect(runtimeInstance.task.submit({
      tasks: [
        { task: "valid member", agent: "aili.implementer", async: false, writeScope: { paths: ["src"] }, formalContext: { changeId: "exact-change" }, continuationAudit: continuationAudit() },
        { task: "invalid formal member", agent: "aili.implementer", formalContext: { changeId: "exact-change" } },
      ],
    })).rejects.toThrow(/explicit boolean async/);
    await expect(runtimeInstance.task.submit({
      tasks: [
        { task: "valid root member", agent: "aili.implementer", async: false, writeScope: { paths: ["src"] }, formalContext: { changeId: "exact-change" }, continuationAudit: continuationAudit() },
        { task: "invalid root member", agent: "aili.implementer", async: false, writeScope: { paths: ["src"] }, formalContext: { changeId: "missing-change" }, continuationAudit: continuationAudit() },
      ],
    })).rejects.toThrow(/formalContext|exact v1/i);
    expect(runtimeInstance.journal.getState().agents).toEqual({});
    expect(executions).toBe(0);

    const formal = await runtimeInstance.task.submit({
    ...
    const ordinary = await runtimeInstance.task.submit({ task: "ordinary compatibility", async: false });
  ...
  it("carries the exact board identity from orchestration through allocation and one audited hub continuation", async () => {
    ...
    expect(plan.taskRequest.writeScope).toEqual({ paths: ["src"], resources: [] });
    ...
    const result = await runtimeInstance.task.submit(plan.taskRequest);
...
describe("formal Runtime restart reconciliation planner", () => {
  ...
  it("runs only through the explicit ROSE entry, applies portable evidence atomically, and is idempotent", async () => {
    ...
    const submitted = await runtimeInstance.task.submit({
  ...
  it("reconciles every same-identity follow-up turn without reusing the initial canonical result", async () => {
    for (const followUpState of ["running", "completed", "failed", "interrupted"] as const) {
      const project = join(scratch, `follow-up-${followUpState}`);
      await mkdir(join(project, "openspec", "changes"), { recursive: true });
      await writePair(project, "fixture-change");
      const parentFile = join(project, "parent.jsonl");
      await writeFile(parentFile, "fixture parent\n");
      const create = () => PersistentAgentRuntime.create({
        parentSessionPath: parentFile,
        parentId: `parent-follow-up-${followUpState}`,
        cwd: project,
        execute: async (input) => {
          const output = canonicalResult();
          persistAssistant(input, output);
      ...
      let runtimeInstance = await create();
      const submitted = await runtimeInstance.task.submit({
        task: "initial canonical result",
        agent: "aili.implementer",
        async: false,
        workspace: "shared",
        writeScope: { paths: ["src"], resources: [] },
      ...
      const agentId = submitted.results[0]!.agentId;
      expect(await runtimeInstance.hub.park(agentId)).toBe(true);
      const receipt = await runtimeInstance.hub.execute({
        action: "send",
        agentId,
        message: `Follow up with state ${followUpState}.`,
        continuationAudit: continuationAudit(),
      }) as { turnId: string };
      if (followUpState === "completed" || followUpState === "failed") {
        await runtimeInstance.hub.settleMessageTurn(agentId, receipt.turnId, followUpState);
      } else if (followUpState === "interrupted") {
        await runtimeInstance.shutdown();
        runtimeInstance = await create();
        expect(runtimeInstance.journal.getState().turns[receipt.turnId]?.state).toBe("interrupted");
      }
      ...
      if (followUpState === "running") {
        expect(result).toMatchObject({ status: "preserved", plan: { decisions: [{ decision: "waiting" }] }, updates: [] });
        expect(await readFile(join(changeRoot, "formal-task-board.md"), "utf8")).toBe(runningBoard);
      } else {
        expect(result).toMatchObject({ status: "applied", plan: { decisions: [{ decision: "blocked", operation: { to: "blocked" } }] } });
        expect(await readFile(join(changeRoot, "formal-task-board.md"), "utf8")).toContain("  - Status: `blocked`");
        if (followUpState === "completed") {
          if (!("plan" in result) || !result.plan) throw new Error("blocked follow-up reconciliation must retain its plan");
          expect(result.plan.decisions[0]!.reason).toContain("initial result cannot be reused");
        }
      }
      await runtimeInstance.shutdown();
    }
  ...
  it("filters candidates by the full current change/package/role identity and blocks only two exact current candidates", async () => {
    ...
    for (const changeId of ["other-change", "fixture-change"]) {
      await runtimeInstance.task.submit({
