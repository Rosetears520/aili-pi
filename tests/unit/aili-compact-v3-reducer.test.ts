...
function planningCall(id: string, toolName: "aili_compact_status" | "aili_compact"): SessionLikeEntry {
  return {
    id,
    type: "message",
    message: { role: "assistant", toolCalls: [{ id, name: toolName }] },
  };
}

function planningResult(id: string, callId: string, toolName: "aili_compact_status" | "aili_compact"): SessionLikeEntry {
  return {
    id,
    type: "message",
    message: {
...
describe("AILI Compact v3 reducer integration", () => {
  ...
  it("recomputes transparent gap proofs from the immutable current-epoch replay slice", () => {
    const left = message("gap:left");
    const statusCall = planningCall("gap:status-call", "aili_compact_status");
    const statusResult = planningResult("gap:status-result", statusCall.id, "aili_compact_status");
    const right = message("gap:right");
    const sourceEntries = [left, statusCall, statusResult, right];

    let state = empty();
    const first = t1Transaction(state, "gap:t1-left", [left.id], 1);
    state = applied(state, first, new Map([[left.id, 1]]));
    ...
    entries.push(custom("gap:first", publicTransaction(entries, empty(), first)));
    entries.push(custom("gap:second", publicTransaction(entries, firstState, second)));
    entries.push(custom("gap:parent", publicTransaction(entries, state, parent)));

    const replay = reduceV3LifecycleState(entries);
    expect(replay.diagnostics).toEqual([]);
    expect(replay.acceptedTransactionCount).toBe(3);
    expect(replay.state?.blocks.get("gap:t2")).toMatchObject({
      firstLeafOrdinal: 1,
      lastLeafOrdinal: 4,
      leafCount: 2,
    });
    ...
    const forgedReplay = reduceV3LifecycleState([
      ...entries.slice(0, -1),
      custom("gap:forged-parent", publicTransaction(entries.slice(0, -1), state, forgedParent)),
    ]);
    expect(forgedReplay.acceptedTransactionCount).toBe(2);
    expect(forgedReplay.state?.blocks.has("gap:t2")).toBe(false);
    expect(forgedReplay.diagnostics).toEqual([
