...
): BranchSessionEntry => ({ id, type: "message", ...(parentId !== undefined ? { parentId } : {}), message: { role, content } });
...
describe("AILI Compact BranchIndex cold and incremental paths", () => {
  it("accepts Pi's null parent sentinel only for a cold root or the first append into an empty snapshot", () => {
    const coldRoot = message("root", "user", "root question", null);
    const cold = coldBuildBranchIndex({ key: key("root"), entries: [coldRoot] });
    expect(cold.ok).toBe(true);
    if (!cold.ok) return;
    expect(getIndexedEntry(cold.snapshot, "root")?.entry).toBe(coldRoot);
    expect(getIndexedEntry(cold.snapshot, "root")?.parentId).toBeUndefined();

    const empty = coldBuildBranchIndex({ key: key("root"), entries: [] });
    expect(empty.ok).toBe(true);
    if (!empty.ok) return;
    const appendedRoot = message("appended-root", "user", "first appended question", null);
    const appended = appendBranchIndex(empty.snapshot, {
      entries: [appendedRoot],
      nextBranchLeafId: "appended-root",
    });
    expect(appended.ok).toBe(true);
    if (!appended.ok) return;
    expect(getIndexedEntry(appended.snapshot, "appended-root")?.entry).toBe(appendedRoot);
    expect(getIndexedEntry(appended.snapshot, "appended-root")?.parentId).toBeUndefined();
  });

  it("rejects null parents after the true root on cold and append paths", () => {
    expect(coldBuildBranchIndex({
      key: key("later-null"),
      entries: [message("root", "user", "root", null), message("later-null", "assistant", "invalid", null)],
    })).toEqual(expect.objectContaining({ ok: false, code: "invalid-entry", diagnostic: "invalid parent for later-null" }));
    ...
    expect(appendBranchIndex(nonEmpty.snapshot, {
      entries: [message("later-null", "assistant", "invalid", null)],
      nextBranchLeafId: "later-null",
    })).toEqual(expect.objectContaining({ ok: false, code: "invalid-entry", diagnostic: "invalid parent for later-null" }));

    const empty = coldBuildBranchIndex({ key: key("root"), entries: [] });
    expect(empty.ok).toBe(true);
    if (!empty.ok) return;
    expect(appendBranchIndex(empty.snapshot, {
