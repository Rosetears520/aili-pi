...
describe("frozen context and retry upstream inventory", () => {
  it("retains complete billion-context source metadata and the bounded AILI routing patch", async () => {
    ...
    const evaluator = await readFile(new URL("upstream/billion-context-pi/src/pressure-evaluator.ts", root), "utf8");
    ...
    expect(source).toContain("createAcpPressureEvaluator");
    expect(evaluator).toContain("renderTags: \"none\"");
    expect(evaluator).not.toContain("store.save");
    expect(runtime).toContain("DEFAULT_OWNERSHIP_ROUTER");
    expect(runtime).toContain("createAcpPressureEvaluator");
