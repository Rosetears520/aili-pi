...
describe("provenance and SBOM", () => {
  it("records the adapted snapshot and all four pinned runtime dependencies", async () => {
    ...
    expect(provenance.sources.filter((item: { status: string }) => item.status === "adapted")).toHaveLength(1);
