...
describe("provenance and SBOM", () => {
  it("records adapted sources and all seven pinned runtime dependencies", async () => {
    ...
    expect(provenance.sources.filter((item: { status: string }) => item.status === "dependency")).toHaveLength(7);
