...
import {
  ...
  planFormalHubJoin,
  planFormalPackageExecution as planFormalPackageExecutionRuntime,
  type FormalPackageExecutionPlanInput,
...
type FormalPlanInput = Extract<FormalPackageExecutionPlanInput, { lifecycle: FormalLifecycleSnapshot }>;

function planFormalPackageExecution(
  input: Extract<FormalPackageExecutionPlanInput, { lifecycle: { kind: "ordinary" } }>
    | (Omit<FormalPlanInput, "writeScope"> & { writeScope?: FormalPlanInput["writeScope"] }),
) {
  return input.lifecycle.kind === "ordinary"
    ? planFormalPackageExecutionRuntime(input as Extract<FormalPackageExecutionPlanInput, { lifecycle: { kind: "ordinary" } }>)
    : planFormalPackageExecutionRuntime({ writeScope: { paths: ["src"], resources: [] }, ...input });
}
...
describe("formal lifecycle orchestration planner", () => {
  ...
  it("fails closed for a mutation-capable formal role without an explicit write scope", async () => {
    const profiles = await loadRoleProfiles();
    const result = planFormalPackageExecution({
      lifecycle: lifecycle(profiles, [packageBlock()]),
      packageId: "P-01",
    ...
    expect(codes(result)).toContain("WRITE_SCOPE_REQUIRED");
    expect(result).not.toHaveProperty("taskRequest");
  });
